...
# SQLite persistence is not guaranteed on Vercel serverless functions.
# For production, use a hosted database.
# Using /tmp/ for temporary storage if necessary, but ideally use an external DB.
...
def seed_data():
    ...
    if User.query.first() is None:
        ...
        users = [
            ...
            User(aadhaar="777788889999", phone="9988776655", email="dhanishkanth1122@gmail.com") 
        ]
        db.session.add_all(users)
        db.session.commit()
        print("Seeding complete.")

# Initialize Tables (Careful with SQLite on Vercel)
# In serverless environments, initialize DB only if it doesn't exist
# or handle it as part of initial setup.
...
def update_status(id):
    ...
    # In serverless, async email via threading is unreliable. 
    # Synchronous send is used here for simplicity.
    try:
